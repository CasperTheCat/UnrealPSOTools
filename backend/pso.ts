...
async function entry()
{
    ...
    app.post("/api/pco/version/after", bodyParser.json({ limit: '99mb', strict: true }), async (req, res) => 
    {
        try
        {
            ...
            if (
                ...
                "version" in jsonbody && typeof jsonbody["version"] === 'string')
            {
                ...
                if ("platform" in jsonbody && typeof jsonbody["platform"] === 'string')
                {
                    platform = jsonbody["platform"];
                }

                if ("shadermodel" in jsonbody && typeof jsonbody["shadermodel"] === 'string')
                {
                    shaderModel = jsonbody["shadermodel"];
                }            
                
                if ("tag" in jsonbody && typeof jsonbody["tag"] === 'string')
                {
                    extag = jsonbody["tag"];
                }        

                if ("type" in jsonbody && typeof jsonbody["type"] === 'string')
                {
    ...
    app.post("/api/pco/date/after", bodyParser.json({ limit: '99mb', strict: true }), async (req, res) => 
    {
        try
        {
            ...
            if (
                "project" in jsonbody && typeof jsonbody["project"] === 'string' &&
                "machine" in jsonbody && typeof jsonbody["machine"] === 'string' &&
                "date" in jsonbody && typeof jsonbody["date"] === 'string'
                )
            {
                let ProjectUUID: Buffer = Buffer.from(jsonbody["project"], 'base64');
                let MachineUUID: Buffer = Buffer.from(jsonbody["machine"], 'base64');
    ...
    app.put('/api/machines', bodyParser.json({ limit: '99mb', strict: true }),// ensureLoggedIn(), 
        async (req, res) => 
        {
            ...
            if(userid && "org" in jsonbody && typeof jsonbody["org"] === 'string')
            {
                let result: number = -1;
                let organisation: Buffer = Buffer.from(req.body.org, "base64");

                if ("machinename" in jsonbody && typeof jsonbody["machinename"] === 'string')
                {
                    result = await psoDB.AddMachine(userid, req.body.machinename, organisation);
                }
                else if (
                    "project" in jsonbody && typeof jsonbody["project"] === 'string' && 
                    "machineuuid" in jsonbody && typeof jsonbody["machineuuid"] === 'string' && 
                    "validfrom" in jsonbody && typeof jsonbody["validfrom"] === 'string' && 
                    "validuntil" in jsonbody && typeof jsonbody["validuntil"] === 'string' && 
                    "canSubmit" in jsonbody && typeof jsonbody["canSubmit"] === 'boolean' && 
                    "canPull" in jsonbody && typeof jsonbody["canPull"] === 'boolean'
                    )
                {
                    let project: Buffer = Buffer.from(req.body.project, "base64");
        ...
        async (req, res) => 
        {
            try 
            {
                ...
                if(userid && "org" in jsonbody && typeof jsonbody["org"] === 'string')
                {
                    ...
                    if ("projectname" in jsonbody && typeof jsonbody["projectname"] === 'string')
                    {
    ...
    app.delete('/api/machines', bodyParser.json({ limit: '99mb', strict: true }),// ensureLoggedIn(), 
        async (req, res) => 
        {
            ...
            if(userid > 0 && "machineuuid" in jsonbody && typeof jsonbody["machineuuid"] === 'string')
            {
                let result: number = -1;
                let machineID: Buffer = Buffer.from(req.body.machineuuid, "base64");

                if ("org" in jsonbody && typeof jsonbody["org"] === 'string')
                {
                    let Organisation: Buffer = Buffer.from(req.body.org, "base64");
                ...
                else if ("project" in jsonbody && typeof jsonbody["project"] === 'string')
                {
    ...
    app.delete('/api/projects', bodyParser.json({ limit: '99mb', strict: true }),// ensureLoggedIn(), 
        async (req, res) => 
        {
            ...
            if(userid > 0 && "projectuuid" in jsonbody && typeof jsonbody["projectuuid"] === 'string')
            {
                let result: number = -1;
    ...
    app.post('/api/pco/new/', bodyParser.json({ limit: '99mb', strict: true }),// ensureLoggedIn(), 
        async (req, res) => 
        {
            ...
            if (
                ...
                "machine" in jsonbody && typeof jsonbody["machine"] === 'string' &&
                "version" in jsonbody && typeof jsonbody["version"] === 'string' &&
                "data" in jsonbody && typeof jsonbody["data"] === 'string' &&
                "shadertype" in jsonbody && typeof jsonbody["shadertype"] === 'string'
                )
            {
